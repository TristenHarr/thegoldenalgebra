...
The first family of series we analyze are those weighted by polynomial powers of the index. Since it is proven in Lemma~\ref{lem:convergence} that $|\LambdaG|<1$, the constant is a valid argument for the standard formula for polynomial-weighted geometric series. This sum is given in terms of the Eulerian polynomials, $A_k(x)$ \cite{AandS}. For instance, the first two Eulerian polynomials are $A_0(x)=1$ and $A_1(x)=x$. The general identification for our constant is as follows:
...
The second family of series are those weighted by reciprocal powers of the index. These are identified by definition with the Polylogarithm special function \cite{Lewin}.
