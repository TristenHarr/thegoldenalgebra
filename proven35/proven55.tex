...
We define two real constants, $T$ and $J$, as the unique solution to a pair of equations motivated by a classical geometric problem (see Appendix~A).
...
The first family of series we analyze are those weighted by polynomial powers of the index. Since it is proven in Lemma~\ref{lem:convergence} that $|\LambdaG|<1$, the constant is a valid argument for the standard formula for polynomial-weighted geometric series. This sum is given in terms of the Eulerian polynomials, $A_k(x)$. For instance, the first two Eulerian polynomials are $A_0(x)=1$ and $A_1(x)=x$. The general identification for our constant is as follows:
