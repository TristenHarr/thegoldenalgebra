...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1]
        % Define segment lengths based on a total length of 8 units for 1/2
        \def\totalLen{8}
        \def\phiVal{1.61803398875}
        \def\Jlen{\totalLen / (\phiVal + 1)}
        \def\Tlen{\Jlen * \phiVal}
        ...
        \draw[thick] (\Tlen, -0.15) -- (\Tlen, 0.15);
        
        % Add labels for T and J
        \node[above=3pt, blue] at (\Tlen/2, 0) {$T$};
        \node[above=3pt, red] at (\Tlen + \Jlen/2, 0) {$J$};
        
        % Color the segments
        \draw[line width=1.5pt, blue] (0,0) -- (\Tlen, 0);
        \draw[line width=1.5pt, red] (\Tlen,0) -- (\totalLen,0);
    \end{tikzpicture}
